...
voice_location = "C:\\Users\\Mark\\Desktop\\xyloweft\\voices"
processed_dir = os.path.join(voice_location, "processed")
...
class VoiceFileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        if not event.is_directory and event.src_path.lower().endswith('.m4a'):
            print(f"\n检测到新语音文件: {event.src_path}")
            time.sleep(1)  # 等待文件完全写入
            self.process_func(event.src_path)

def process_audio_to_ai(file_path, voice_model, client, chat, txt, jsons, count):
    try:
        # 语音转文字
        print("正在转换语音...")
...
["sphere":"一个球体,一个半圆绕直径所在直线旋转一周所成的空间几何体,任何可被抽象成球体的物件", "cuboid":"一个所有面等角的六面体,任何可被直接抽象成等角六面体的物体", "pyramid":"一个多面棱台,可为棱柱,棱锥等,任何可直接被抽象成棱柱的物体"]
...
有时候你会收到一些奇奇怪怪的非例题类提示词,请保持你的功能尽可能去生成出需要的体,如将几个体堆叠成一栋大楼的形状。


同时,我们在此对一个json里的每个体的命名方式有所要求:详情格式为 [体的geotype的首字母大写,S为简单体,C为复杂体,V为虚拟体]_[体的type的首字母小写]_[体在这句话里出现的绝对顺序，即第几个被生成]_[现在的时间,以下是现在的时间],如一个在30号15点34分40秒生成的第一个立方体会被其名为S_s_1_30153440
...
""")
            ...
            time = main.get_current_time()
            response = chat.send_message(f"""
现在有另一句语句供你处理,这是你处理的第{count}句话,这句语句是上一句语句的延申,是第一段话的附加条件,请保留第一次生成的记忆,抽象出这一句话的json供我们处理。注意,这一句话是第一句话的延伸,所以可能会包含对第一句话的json的更改。

以下是目前收到的所有对话和目前你返还给我们过的最后一个json,请根据这些信息创造一个包含所有对话信息的新json。请保持之前要求的命名规律,但请记住这是一句新的话,我会再次提供给你现在的时间，请使用这个时间。别忘了，生成名字的时候使用的是绝对顺序，即如果上一句话已经生成了三个体则这句话的体的绝对顺序从4起算
...
def run():
    ...
    # 设置文件监控
    event_handler = VoiceFileHandler(lambda file_path: process_audio_to_ai(
        file_path, voice_model, client, chat, txt, jsons, count))
    observer = Observer()
    observer.schedule(event_handler, voice_location, recursive=False)
    observer.start()
    
    print(f"系统已启动，正在监控文件夹: {voice_location}")
    print("放入.m4a文件将自动处理，也可以手动输入命令")
    
    try:
        while True:
            if count == 0:
                question = input("\n欢迎使用，输入'run'开始交互，或直接放入.m4a文件: ")
                if question == "break":
                    break
                if question == "run":
                    count += 1
                    question = input("请输入第一个问题: ")
                    if question == "break":
                        break
                    count = process_audio_to_ai(question, voice_model, client, chat, txt, jsons, count - 1)
            else:
                question = input(f"请输入第{count}个问题(或'break'退出): ")
                if question == "break":
                    break
                count = process_audio_to_ai(question, voice_model, client, chat, txt, jsons, count - 1)
